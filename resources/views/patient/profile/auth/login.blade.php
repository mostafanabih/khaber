				...
				<div class="login-form">
					<form action="{{ route('login') }}" method="post">
						...
						<div class="form-row row">
                            ...
                            @endif
							<button type="submit" class="btn btn-primary">{{ $websiteLang->where('lang_key','login')->first()->custom_lang }}</button>
						</div>
					</form>
					<div class="mt_20">
						<a href="{{ route('forget.password') }}" class="link">{{ $websiteLang->where('lang_key','forget_your_pass')->first()->custom_lang }}</a><br>
						<a href="{{ route('register') }}" class="link">{{ $websiteLang->where('lang_key','reg_here')->first()->custom_lang }}</a>
					</div>
				</div>
			</div>
		</div>
