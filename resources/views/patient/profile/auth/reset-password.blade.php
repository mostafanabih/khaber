				...
				<div class="regiser-form login-form">
					<form action="{{ route('store.reset.password',$token) }}" method="POST">
						...
						<div class="form-row row">
                            ...
                            <div class="form-group col-12">
								<label for="password-confirmation">{{ $websiteLang->where('lang_key','confirm_pass')->first()->custom_lang }}</label>
							...
							<div class="form-group col-12">
								<button type="submit" class="btn btn-primary">{{ $websiteLang->where('lang_key','reset_pass')->first()->custom_lang }}</button>
							</div>
						</div>
					</form>

					<a href="{{ url('login') }}" class="link">{{ $websiteLang->where('lang_key','login_here')->first()->custom_lang }}</a>
