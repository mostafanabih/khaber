...
@section('title')
<title>{{ $websiteLang->where('lang_key','term_and_cond')->first()->custom_lang }}</title>
@endsection
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.terms-privacy.update',$conditionPrivacy->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            <label for="terms_condition">{{ $websiteLang->where('lang_key','term_and_cond')->first()->custom_lang }}</label>
                            <textarea class="summernote" id="terms_condition" name="terms_condition">{{ $conditionPrivacy->terms_condition }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="privacy_policy">{{ $websiteLang->where('lang_key','privacy_policy')->first()->custom_lang }}</label>
